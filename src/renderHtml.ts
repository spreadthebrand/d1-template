...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .warning-screen { position:fixed; inset:0; z-index:100; background:radial-gradient(circle at top,#402606,#030303 48%,#000); display:grid; place-items:center; padding:18px; }
    .warning-box { max-width:500px; border:1px solid rgba(255,92,92,.4); background:rgba(10,10,10,.94); border-radius:28px; padding:22px; box-shadow:0 30px 80px rgba(0,0,0,.7); }
...
<body>
  ...
  <main class="app">
    <header class="hero">
      ...
      <div class="risk-banner">Educational trading assistant only. Trading is risky. This app does not provide financial advice and does not guarantee results.</div>
    </header>
    ...
    <section id="education" class="screen">
      ...
      <div class="notice">Optional alerts are sent only for High confidence signals. Tokens stay in LocalStorage on this device and are never sent anywhere except Telegram's bot API when you press Send/Test or generate a High signal.</div>
    </section>

    <section id="settings" class="screen">
      <h2 class="section-title">Admin / Settings</h2>
      <div class="card"><div class="form" id="settingsForm"></div><div class="btnrow" style="margin-top:14px;"><button id="saveSettings" class="btn primary">Save Settings</button><button id="resetDay" class="btn danger">Reset Daily Stats</button></div></div>
    </section>
  </main>
...
(function(){
  ...
  var state = load('oneSvState', { settings: defaultSettings, input: defaultInput, trades: [], latest: null, lockedUntil: '', paused: false });
  ...
  function load(key, fallback){ try { return Object.assign({}, fallback, JSON.parse(localStorage.getItem(key) || '{}')); } catch(e){ return fallback; } }
  function save(){ localStorage.setItem('oneSvState', JSON.stringify(state)); }
  ...
  function cap(conf){ return MODELS[state.settings.model].max === 'Medium' && conf === 'High' ? 'Medium' : conf; }
  ...
  function sendTelegram(sig){
    var s=state.settings; if(!s.botToken || !s.chatId || sig.confidence!=='High') return;
    var text='1SV FAST TRADE ALERT\nModel: '+MODELS[sig.model].name+'\nSignal: '+sig.signal+'\nExpiry: '+sig.expiry+'\nConfidence: '+sig.confidence+'\nReason: '+sig.reason+'\nRisk Status: '+riskStatus().text+'\nReminder: Manual confirmation only.';
    fetch('https://api.telegram.org/bot'+encodeURIComponent(s.botToken)+'/sendMessage',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:s.chatId,text:text})}).catch(function(){});
  }
  ...
  function renderDashboard(){
    ...
    document.getElementById('dashboardCards').innerHTML = [
      card('Status',r.text,r.locked?'red':(r.paused?'blue':'green')), card('Daily P/L',money(r.pl),r.pl>=0?'green':'red'), card('Daily target',money(state.settings.target),'gold'), card('Daily loss stop',money(state.settings.maxLoss),'red'), card('Trades',r.trades+' / '+state.settings.maxTrades,''), card('Win / Loss streak',r.wins+'W • '+r.losses+'L',''), card('Model',MODELS[state.settings.model].name,'gold'), card('Trade size',riskTradeSize(),'blue')
    ].join('');
  ...
  document.getElementById('generateSignal').onclick=function(){ state.latest=analyze(); sendTelegram(state.latest); refresh(); nav('home'); };
  ...
  document.getElementById('exportCsv').onclick=function(){ var rows=[['Date/time','Model used','Asset','Timeframe','Expiry','Signal','Confidence','Result','Profit/loss','Notes']].concat(state.trades.map(function(t){return [t.date,MODELS[t.model].name,t.asset,t.timeframe,t.expiry,t.signal,t.confidence,t.result,t.pl,t.notes];})); var csv=rows.map(function(r){return r.map(function(v){return '"'+String(v==null?'':v).replace(/"/g,'""')+'"';}).join(',');}).join('\n'); var a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download='1sv-trade-journal.csv'; a.click(); URL.revokeObjectURL(a.href); };
  document.getElementById('acceptRisk').onclick=function(){ localStorage.setItem('oneSvRiskAccepted','yes'); document.getElementById('riskWarning').classList.add('hidden'); };
  if(localStorage.getItem('oneSvRiskAccepted')==='yes') document.getElementById('riskWarning').classList.add('hidden');
