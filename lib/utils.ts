...
export const hashValue = (value = "") => crypto.createHash("sha256").update(`${process.env.NEXTAUTH_SECRET ?? "local-dev"}:${value}`).digest("hex");
export const safeJson = (value: unknown) => JSON.parse(JSON.stringify(value));
